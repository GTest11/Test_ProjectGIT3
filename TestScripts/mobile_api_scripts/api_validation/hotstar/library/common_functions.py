...
class common_functions(object):
    ...
    def verify_playback(self):
        ...
        playback = False
        try:
            coord = self.config.c_detect_motion[self.get_dut_name()]
            start_time = self.get_time_stamp()
            playback = dut.validator.DetectMotion(*coord)
            end_time = self.get_time_stamp()
            if playback:
                status = "PASSED"
            else:
                status = "FAILED"
            self.report_api_status(start_time, end_time, "DetectMotion", status, ss_required=False)
