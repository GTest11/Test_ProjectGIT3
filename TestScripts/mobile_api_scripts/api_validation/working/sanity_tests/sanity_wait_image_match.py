...
class api_test(apiTestBase):
    ...
    def run_api_test(self):
        ...
        try:
            ...
            capture_time1 = self.lib.gettimestamp()
            if not self.dut.validator.CaptureImage(self.config.C_WAIT_IMG_MATCH[0], self.config.C_WAIT_IMG_MATCH[1], self.config.C_WAIT_IMG_MATCH[2], self.config.C_WAIT_IMG_MATCH[3],
                                                   self.config.img_wait_im_match, self.config.CAP_IMG_DEFAULT_QUALITY, 2):
                self.logger.Log("CaptureImage API Failed. Hence, skipping WaitImageMatch API")
            else:
                capture_time2 = self.lib.gettimestamp()
                self.lib.report_api_status(capture_time1, capture_time2, "CaptureImage", "PASSED",
                                           ss_required=True)
                test_data = self.lib.get_test_params(self.config.sanity_wait_image_match_params,self.config.wait_image_match_params)
