...
#                 Right-left swipe
#                 Left-right swipe
#                 Swipe on a small area
#                 Both coordinates are same
#                 Swipe until page ends
...
class api_test(apiTestBase):
    ...
    def run_api_test(self):
        test_result = True
        params = self.config.swipe_params
        swiped, same = False, False
        counter = 25
        try:
            for param in params.keys():
                ...
                self.lib.report("Swipe: {}".format(params[param][0]), status)
