...
class api_test(apiTestBase):
    ...
    def swipe_screen(self, coord):
        ...
        try:
            ...
            swiped = self.dut.VerticalSwipe(*coord)
            ...
            if swiped:
                ...
                status = "FAILED"
            self.lib.report_api_status(start_time, end_time, "VerticalSwipe", status, ss_required=True)
    ...
    def run_api_test(self):
        ...
        test_result = True
        params = self.config.v_swipe_params
        swiped, same = False, False
        counter = 50
        ...
        try:
            for param in params.keys():
                ...
                self.lib.report("VerticalSwipe: {}".format(params[param][0]), status)
    ...
    def cleanup(self):
        '''
        Vertical swipe leaves system quick access overlay open on the screen. This provokes further executions to
        get failed. Hence calling a bottom-up swipe to clear the screen

        :return: None
        '''

        self.dut.VerticalSwipe(*self.config.C_BOTTOM_UP_V_SWIPE)
        time.sleep(3)
