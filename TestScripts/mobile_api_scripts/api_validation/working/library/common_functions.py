...
class common_functions(object):
    ...
    def get_dut_platform(self):
        ...
        try:
            ...
            logger.Log("dut_platform: " + str(dut_platform))
            return dut_platform
    ...
    def cleartext(self, elm_name):
        ...
        try:
            ...
            if not gettext or gettext == self.config.yt_default_text:
                status = "PASSED"
                cleared = True
            else:
                status = "FAILED"
    ...
    def search_in_youtube(self):
        ...
        try:
            ...
            keys_sent = self.send_keys(self.config.search_txt_box, keys=self.config.search_text)
            if not keys_sent:
                return False
