...
# Description       :   DoubleTapElement  API testing.
...
``````````````````````````````````````````````````````````````````````````
Author			    : Arathy P S
Function Name	  	: test_double_tap_element()
Description		    : Function to test the DoubleTapElement API
...
def test_double_tap_element():
    try:
        ...
        if com_lib.dut.DoubleTapElement(com_lib.MobileScriptingLibrary.Constants.ElementType.XPath, tap_element_id):
            end_time = com_lib.str_to_milli_second_time()
            com_lib.time.sleep(3)
            com_lib.dut.validator.QuickCapture("DoubleTap_after_imageName")
            tap_time = com_lib.get_duration(start_time, end_time)
            com_lib.commit_step_result("API: Time taken by DoubleTapElement API", str(tap_time))
            com_lib.commit_step_result("API: DoubleTapElement API", "PASSED")
            fn_return = True
        else:
            end_time = com_lib.str_to_milli_second_time()
            tap_time = com_lib.get_duration(start_time, end_time)
            com_lib.commit_step_result("API: Time taken by DoubleTapElement API", str(tap_time))
            com_lib.commit_step_result("API: DoubleTapElement API", "FAILED")
    except Exception as e:
        com_lib.log_error("Exception raised in test_double_tap_element function : " + str(e))
    finally:
        return fn_return
    ...
    try:
        ...
        if init_app_time > 0:
            if "Android" == platform:
				...
				element_id = com_lib.ios_conf.ios_youTube_home				
            if not com_lib.dut.WaitForElement(com_lib.MobileScriptingLibrary.Constants.ElementType.XPath, element_id, 20):
                ...
                status = test_double_tap_element()
        else:
