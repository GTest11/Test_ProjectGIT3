...
# Date              :   16-03-2018
# FE Version        :   5.0.9.4
# APIs covered      :   ClearText, Click(with index), SendKeys
# Description       :   ClearText API testing.
...
``````````````````````````````````````````````````````````````````````````
Author			    : Rohith P V
Function Name	  	: test_cleartext()
Description		    : Function to test the ClearText API
...
def test_cleartext():
    try:
        ...
        if str(com_lib.dut.ReadProperty(3)) == "Android":
            xpath_cleartext = com_lib.android_conf.ELEMENT_XPATH_CLEARTEXT
        else:
            xpath_cleartext = com_lib.ios_conf.ELEMENT_XPATH_CLEARTEXT
        if com_lib.youtube_search():
            start_time = com_lib.str_to_milli_second_time()
            com_lib.dut.ClearText(com_lib.MobileScriptingLibrary.Constants.ElementType.XPath,xpath_cleartext)
            com_lib.dut.validator.QuickCapture("ClearText_imageName")
            end_time = com_lib.str_to_milli_second_time()
            ClearText_time = com_lib.get_duration(start_time, end_time)
            com_lib.commit_step_result("API: Time taken by ClearText API ", str(ClearText_time))
            if(com_lib.validate_screen(com_lib.constant.CHKPNT_CLEARTEXT)):
                fn_return = True
                com_lib.log_info("Text cleared successfully")
                com_lib.commit_step_result("API: ClearText API", "Passed")
                com_lib.commit_step_result("API: Reason", "Text cleared")
            else:
                com_lib.log_info("Clear text failed")
                com_lib.commit_step_result("API: ClearText API", "Failed")
                com_lib.commit_step_result("API: Reason", "Text is not cleared")
        else:
            com_lib.log_info("youtube_search function failed")
            com_lib.commit_step_result("youtube_search function", "Failed")
    except Exception as e:
        com_lib.log_error("Exception raised in test_cleartext function : " +str(e))
    finally:
        return fn_return
