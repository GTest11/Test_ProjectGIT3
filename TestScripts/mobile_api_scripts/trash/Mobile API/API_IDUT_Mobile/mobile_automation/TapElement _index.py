...
``````````````````````````````````````````````````````````````````````````
Author			    : Arathy P S
Function Name	  	: test_tap_element_with_index()
Description		    : Function to test the TapElement with index API
...
def test_tap_element_with_index():
    try:
        if platform == "Android":
            tap_element_id = com_lib.android_conf.android_APP_TAP_ID
            tap_index = com_lib.android_conf.android_APP_TAP_INDEX
        else:
            tap_element_id = com_lib.ios_conf.ios_APP_TAP_ID
            tap_index = com_lib.ios_conf.ios_APP_TAP_INDEX
        start_time = com_lib.str_to_milli_second_time()
        if com_lib.dut.TapElement(com_lib.MobileScriptingLibrary.Constants.ElementType.XPath,
                                  tap_element_id, tap_index):
            ...
            com_lib.commit_step_result("API: TapElement API with index", "PASSED")
            return True
        else:
            com_lib.commit_step_result("API: TapElement API with index", "FAILED")
            return False
    except Exception as e:
        com_lib.log_error("Exception raised in test_tap_element_with_index function : " + str(e))
    ...
    try:
        ...
        if init_app_time > 0:
            ...
            if not com_lib.dut.WaitForElement(com_lib.MobileScriptingLibrary.Constants.ElementType.XPath, element_id, 20):
                ...
                status = test_tap_element_with_index()
        else:
