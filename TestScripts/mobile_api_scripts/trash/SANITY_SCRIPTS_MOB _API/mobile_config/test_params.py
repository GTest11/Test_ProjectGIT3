from library.common_functions import dut, get_image_resolution, logger
...
try:
    ...
    logger.Error("Exception while getting MAX_WIDTH, MAX_HEIGHT" + str(e))
    MAX_WIDTH, MAX_HEIGHT = 500, 500
finally:
    # MAX_WIDTH, MAX_HEIGHT = 720, 1000
    pass
...
download_opt_below_video = "yt_download_pgm" # check point name required
...
# ----------------------- WaitForCheckpoint
#     CheckpointScreenToWait is not being used in the script as script uses another checkpoint
INITIAL_DELAY = 2


wait_for_chkpt_params =   {
    0:[
        "Valid parameters, small time to wait, 3 sec.", API_PASS,
        (CheckpointScreenToWait, 3, 1)
    ],
    1:[
        "Valid parameters, screen transition to the checkpoint screen.", API_PASS,
        (download_opt_below_video, TIME_TO_WAIT, INITIAL_DELAY)
    ],
...
ref_image_param = (76, 59, 111, 44, ref_img_0, CAP_IMG_DEFAULT_QUALITY, 0)
test_image_param = (76, 59, 111, 44, test_img_0, 100, 2)
