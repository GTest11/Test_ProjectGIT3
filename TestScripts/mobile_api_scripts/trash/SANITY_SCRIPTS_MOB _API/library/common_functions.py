...
import mobile_config.ipad_config as ipad_conf
import mobile_config.iphone_config as iphone_conf

clr.AddReference("MobileScriptingLibrary")
import MobileScriptingLibrary as fe_mob_lib
dut = fe_mob_lib.MobileDUT()
logger = fe_mob_lib.Logger()
scriptPath = os.path.realpath(__file__)
args = sys.argv
dut.Configure(args[1], args[2], args[3], args[4], scriptPath)
logger.Configure(args[1], args[2], args[3], args[4], scriptPath)
chkpt = fe_mob_lib.CheckPoint()
device_config = fe_mob_lib.DeviceConfig()
...
def set_config(app="youtube"):
    ...
    # logger.Log("Config {}".format(config.dut_type))
    return config
...
Input arguments	    : config    : config = fe_mob_lib.DeviceConfig()
...
def youtube_search_click_type_android():
    try:
        if dut.Click(fe_mob_lib.Constants.ElementType.XPath,
           android_conf.android_app_click, constants.youtube_search_index):
            commit_step_result("Click on Search", "PASSED")
            dut.SendKeys(fe_mob_lib.Constants.ElementType.XPath,
...
def nowtv_launch_video():
    try:
        if dut.Click(fe_mob_lib.Constants.ElementType.ClassName,
...
def search_in_youtube(config):
    ...
    try:
        start_time = gettimestamp()
        clicked = dut.Click(fe_mob_lib.Constants.ElementType.XPath, config.X_CLICK_INDEX)
        end_time = gettimestamp()
        if clicked:
            status = "PASSED"
        else:
            status = "FAILED"
        report_elapsed_time(start_time, end_time, "click-(without Index)", status)
        ...
        start_time = gettimestamp()
        keys_sent = dut.SendKeys(fe_mob_lib.Constants.ElementType.XPath, config.X_SENDKEYS_EDIT_BOX, constants.KEYS)
        end_time = gettimestamp()
        if keys_sent:
            status = "PASSED"
        else:
    ...
    try:
        ...
        if search_in_youtube():
            if str(dut.ReadProperty(3)) == "Android":
                ...
                if dut.Click(fe_mob_lib.Constants.ElementType.XPath, ios_conf.ELEMENT_XPATH_CLICK_ENTER):
                    if tap_video():
                        log_info("Video started")
                        fn_status=True
                    else:
...
def start_run(elm_type=fe_mob_lib.Constants.ElementType.XPath, home=None):
    ready = False
    try:
        if init_app():
            time.sleep(10)
...
def report_elapsed_time(t1, t2, api, status=None):
    time_taken = getAPIduration(t1, t2)
    report("API:  Time taken by {} API".format(api), str(time_taken))




def tapelement_if_present(elm_type, elm_name, elm_frndly_name):
    if dut.IsElementPresent(elm_type, elm_name):
        report(elm_frndly_name, "PASSED")
    else:
        report(elm_frndly_name, "FAILED")
        return False

    if dut.TapElement(elm_type, elm_name):
        report(elm_frndly_name, "PASSED")
    else:
        report(elm_frndly_name, "FAILED")
        return False
...
#@Output values		: None
#''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
def log_script_info(file_name, apis_covered):
    log_info("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    log_info("Script Name: {}".format(file_name))
    log_info("Validating APIs - {}".format(apis_covered))
    log_info("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
	
	
#''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
#@Function Name	  	: get_dut_name()
#@Description		: Function to get the DUT name
#@Input arguments	: None
#@Output values		: dut_name  :  return the dut name.
#''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
def get_dut_name():
    try:
        ...
        logger.Error("Exception raised in get_dut_name: " + str(ex))
        return None
    finally:
        return dut_name