# ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
# TestCase ID: QOE4
# TestCase Name: NowTV_QoE_Ent_LIVE_Monitoring_All

# GIVEN I am on Entertainment LIVE page within the NOW TV
# WHEN I play Entertainment channel continuously in configured interval
...
class TC_QOE4(AndroidBase):
    description = "Looping through the Entertainment channels and finding the QoE parameters"
    asset_name = 'Live'
                    ...
                    while True:
                        ...
                        while asset_no_in_screen < confFile.ITERATION: #assumes maximum 5 assets row in
                            if dut.IsElementPresent(MobileScriptingLibrary.Constants.ElementType.XPath,
                                                    common.get_ent_live_tile(asset_no_in_screen)):
                                if (common.Click(dut,MobileScriptingLibrary.Constants.ElementType.XPath,
                                    ...
                                    ent_name = common.GetText(dut,MobileScriptingLibrary.Constants.ElementType.Id,
                                                              confFile.ALL_MOVIES_ITEM_NAME)
                                    logger.Log("Last Played Entertainment " + last_played_ent_name)
                                    if ent_name == last_played_ent_name:
                                        ...
                                        logger.Log("Last Played Entertainment changing to " + ent_name)
                        ...
                        if asset_no_in_screen < confFile.ITERATION:
                            ...
                            i += 1
                            channel = "Entertainment channel " + str(i)
                            ...
                            while not dut.IsElementPresent(MobileScriptingLibrary.Constants.ElementType.XPath,
                                ...
                                logger.Log("Navigated back to Entertainment Live screen successfully")
                                dut.CommitStepResult("[" + channel + "] Navigated back to Entertainment Live screen",
                                                     common.TestResult.PASSED)
                                dut.SetOrientation("Portrait")
                                time.sleep(confFile.MAXTIMEOUT)
                                ...
                                dut.VerticalSwipe(confFile.Swipe_Vertical_BottomToTop_y1,
                                                  ...
                                                  confFile.Swipe_Vertical_BottomToTop_x2,
                                                  confFile.MOVIES_LIVE_SWIPE_TIME)
                        else:  # Completed the Entertainment live assets
                            break
