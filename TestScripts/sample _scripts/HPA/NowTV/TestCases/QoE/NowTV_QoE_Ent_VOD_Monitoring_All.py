# ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
# TestCase ID: QOE5
# TestCase Name: NowTV_QoE_Ent_VOD_Monitoring_All

# GIVEN I am on Entertainment VOD page within the NOW TV
# WHEN I play Entertainment assets continuously in configured interval
...
class TC_QOE5(AndroidBase):
    description = "Looping through the Entertainment VOD assets and finding the QoE parameters"
    asset_name = 'VOD'
        ...
        try:
            ...
            if not dut.WaitForElement(MobileScriptingLibrary.Constants.ElementType.Id,
                ...
                for capped_bw in self.down_bw_ranges:
                    ...
                    if not (common.Click(dut, MobileScriptingLibrary.Constants.ElementType.XPath,
                                         confFile.ENTERTAINMENT_BUTTON)):
                        break

                    time.sleep(confFile.SHORT_WAITTIME)
                    if not (common.Click(dut, MobileScriptingLibrary.Constants.ElementType.XPath,
                                         confFile.ALL_TV_SHOWS_OPTION)):
                        break
                    ...
                    if not dut.WaitForElement(MobileScriptingLibrary.Constants.ElementType.XPath,
                                              confFile.get_all_tv_shows_tile(1),
                    ...
                    while self.test_result:
                        i = 1
                        j = i + confFile.TV_SHOWS_COUNT_IN_ONE_ROW  # first item in next row
                        if (dut.IsElementPresent(MobileScriptingLibrary.Constants.ElementType.XPath,
                                                 confFile.get_all_tv_shows_tile(i))):
                            play_count = i
                        elif (dut.IsElementPresent(MobileScriptingLibrary.Constants.ElementType.XPath,
                                                   confFile.get_all_tv_shows_tile(j))):
                            play_count = j
                        else:
                            break

                        for loop in range(confFile.TV_SHOWS_COUNT_IN_ONE_ROW):
                            item_played += 1

                            common.Click(dut, MobileScriptingLibrary.Constants.ElementType.XPath,
                                         confFile.get_all_tv_shows_tile(play_count))
                            ...
                            if not (dut.IsElementPresent(MobileScriptingLibrary.Constants.ElementType.Id,
                                ...
                                while dut.IsElementPresent(MobileScriptingLibrary.Constants.ElementType.Id,
                                                           confFile.LOADING_SPINNER):
                                    pass
                                logger.Log("Play button is not found. Swiping down")
                            ...
                            if tuning_time > 0:
                                ...
                                pushStatus = cubeAPI.QoePush(common.QoEParams.START_UP_TIME, detect_motion_date_time, tuning_time,
                                                             self.category_type, self.asset_name, capped_bw, self.custom_dict)
                                logger.Log("Startup Time for Movies VOD " + str(item_played) + " : " + str(
                                    tuning_time) + " sec")
                                dut.CommitStepResult(
                                    "[Movies VOD " + str(item_played) + "] Startup time : " + str(tuning_time),
                                ...
                                if buffering_ratio > 0:
                                    ...
                                    dut.CommitStepResult(
                                        ...
                                        common.TestResult.PASSED)
                                    logger.Log("Buffer Ratio for Movies VOD " + str(item_played) + " = " + str(
                                        buffering_ratio))
                                else:
                                    dut.CommitStepResult(
                                        "[Movies VOD " + str(item_played) + "] Buffer ratio = " + str(buffering_ratio),
                            ...
                            while not dut.IsElementPresent(MobileScriptingLibrary.Constants.ElementType.XPath,
                                                           confFile.ALL_TV_SHOWS_TITLE):
                                element_check_count += 1
                                if element_check_count == confFile.MAXCOUNT:
                                    dut.CommitStepResult("Failed to navigate to All Movies menu",
                                                         common.TestResult.FAILED)
                                ...
                                dut.CommitStepResult(
                                    "[Movies VOD " + str(item_played) + "] Navigated back to Movies VOD screen",
                                    common.TestResult.PASSED)
                                dut.SetOrientation("Portrait")
                                time.sleep(confFile.MAXTIMEOUT)
                        ...
                        if not end_reached_flag:
                            dut.VerticalSwipe(confFile.AllTVShows_Swipe_Vertical_BottomToTop_y1,
                                              confFile.AllTVShows_Swipe_Vertical_BottomToTop_y2,
                                              confFile.AllTVShows_Swipe_Vertical_BottomToTop_x2,
                                              confFile.MOVIES_VOD_SWIPE_TIME)
                            time.sleep(confFile.SHORT_WAITTIME)
                        else:
